...
function detectEnvironment() {
  ...
  const hasAaPanel    = dirExists('/www/server/panel') || dirExists('/www/server/nginx') ||
                        __dirname.includes('/www/wwwroot/') || process.cwd().includes('/www/wwwroot/');
  ...
  <main class="pt-32 pb-16 px-4">
    <div class="max-w-3xl mx-auto">
      ...
      <div id="step-1" class="step active">
        ${env.envType === 'vps-aapanel' ? `
        <div class="bg-emerald-950/60 border border-emerald-600 rounded-xl px-5 py-4 mb-6 flex items-start gap-3">
          <span class="text-xl mt-0.5">✅</span>
          <div class="text-sm">
            <p class="text-emerald-300 font-semibold mb-1">aaPanel detected — if this page loaded, port ${port} is already open ✅</p>
            <p class="text-emerald-400/80">If you can't access this page, go to <strong>aaPanel → Security → Firewall</strong> → Add port ${port}</p>
          </div>
        </div>` : ''}
...
function printTerminalInstructions(env, ip, port) {
  ...
  } else if (env.envType === 'vps-aapanel') {
    console.log('  ' + bold(cyan(`  http://${ip}:${port}`)));
    console.log('');
    console.log('  ' + yellow(`  ⚠️  Port ${port} may be blocked by aaPanel firewall.`));
    console.log('  ' + dim(`     To open it: aaPanel → Security → Firewall → Add Rule`));
    console.log('  ' + dim(`     Port: ${port}, Protocol: TCP, Direction: Inbound, Action: Allow`));
    console.log('');
    console.log('  ' + dim(`     Or run: bt firewall add ${port}`));
  } else if (['vps-linux','vps-cpanel','vps-plesk'].includes(env.envType)) {
    console.log('  ' + bold(cyan(`  http://${ip}:${port}`)));
    console.log('');
